...
def random_activation(x: np.ndarray, func_type: str = "linear") -> np.ndarray:
    ...
    if func_type == "sigmoid":   # Avoid overflow issues with large x， if x<0 , return np.exp(x) / (1 + np.exp(x))
...
def random_edge_mapping(parents_data: List[np.ndarray]) -> np.ndarray:
    ...
    chosen_func = np.random.choice(["sigmoid", "sin", "mod","tanh", "softplus", "swish", "gaussian"])
...
def sample_time_series_from_scm(
    *,
    ...
    for n in chosen_nodes:
        ts = node_data[n].astype(np.float32)
        if np.isnan(ts[:10]).all():
            continue
        results.append({
...
if __name__ == "__main__":
    ...
    parser.add_argument("-N", "--num-series", type=int, default=100000, help="total number of series to generate")
    parser.add_argument("-L", "--time-length", type=int, default=512, help="length of each individual series")
    parser.add_argument("-O", "--output-file", type=str, default="testTime.arrow", help="output file name")
    ...
    end_time = time()
    print(f"Generated {len(results)} time series in {end_time - start_time:.2f} seconds.")
